...
def handle_generation(args, save_used=True):
    ...
    status = f"{folx} can be trusted with {treat}"
    if args.generate_some:
        print(status)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        type=int,
        help="Generate x statuses, but don't post them (dry run)",
    )
    parser.add_argument(
        "-c",
        "--count",
    ...
    if args.generate_some:
        print(f"## Generating {args.generate_some} statuses (dry run) ##\n")
        args.dry_run = True
        count = 0
        while count < args.generate_some:
            handle_generation(args=args, save_used=False)
            count += 1
    else:
